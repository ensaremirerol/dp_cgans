...
def cli_gen(
    input_file: str,
    gen_size: int = typer.Option(
        100,
        help="Number of rows in the generated samples file",
    ),
    epochs: int = typer.Option(
        100, help="Number of epochs"
    ),
    ...
    if verbose:
        print(f"🗜️  Model instantiated, fitting...")
    ...
    if generator_output:
        model.save(generator_output)
        if verbose:
            print(
                f"✅ Generator saved in {BOLD}{GREEN}{generator_output}{END}"
            )
...
def cli_version():
    print(
        pkg_resources.get_distribution("dp_cgans").version
    )
